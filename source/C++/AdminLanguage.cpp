		...
		if( findLanguageByName( languageNameString ) == RESULT_OK )
			...
			if( foundLanguageWordItem_ != NULL )
				...
				if( foundLanguageWordItem_->isGrammarLanguageWord() )
					...
					if( ( languageSpecificationItem = foundLanguageWordItem_->firstNonQuestionAssignmentOrSpecificationItem( true, false, false, false, false, NO_COLLECTION_NR, NO_CONTEXT_NR, NO_CONTEXT_NR, NO_CONTEXT_NR, commonVariables_->predefinedNounGrammarLanguageWordItem ) ) != NULL )
