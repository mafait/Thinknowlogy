...
class WordWriteWords
	...
	SpecificationResultType checkAssumptionLevel( SpecificationItem *writeSpecificationItem )
		...
		if( writeSpecificationItem == NULL )
			return myWordItem_->startSpecificationResultErrorInWord( functionNameString, moduleNameString_, "The given write specification item is undefined" );

		if( ( specificationResult = writeSpecificationItem->getAssumptionLevel() ).result != RESULT_OK )
			return myWordItem_->addSpecificationResultErrorInWord( functionNameString, moduleNameString_, "I failed to get the assumption level" );
		...
		if( ( previousAssumptionLevel = specificationResult.assumptionLevel ) > NO_ASSUMPTION_LEVEL )
			...
			if( ( specificationResult = writeSpecificationItem->recalculateAssumptionLevel() ).result != RESULT_OK )
				return myWordItem_->addSpecificationResultErrorInWord( functionNameString, moduleNameString_, "I failed to recalculate the assumption level" );

			if( specificationResult.assumptionLevel != previousAssumptionLevel )
				return myWordItem_->startSpecificationResultErrorInWord( functionNameString, moduleNameString_, "The assumption level of the given write specification item has changed during the process, but it isn't recalculated. So, this specification may have a recalculation or update issue" );
			}
	...
	WriteResultType writeWordsToSentence( bool isWritingCurrentSpecificationWordOnly, unsigned short answerParameter, GrammarItem *definitionGrammarItem, SpecificationItem *writeSpecificationItem )
		...
		if( definitionGrammarItem == NULL )
			return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "The given definition grammar item is undefined" );

		if( !definitionGrammarItem->hasWordType() )
			return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "The given grammar definition has no word type" );

		if( writeSpecificationItem == NULL )
			return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "The given write specification item is undefined" );
...
*/
			...
			case WORD_PARAMETER_ADVERB_ASSUMPTION_MAYBE:
				if( isSelfGeneratedAssumption )
					...
					if( ( specificationResult = checkAssumptionLevel( writeSpecificationItem ) ).result != RESULT_OK )
						return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to check the assumption level for 'maybe'" );
			...
			case WORD_PARAMETER_ADVERB_ASSUMPTION_POSSIBLY:
				if( isSelfGeneratedAssumption )
					{
					if( ( specificationResult = checkAssumptionLevel( writeSpecificationItem ) ).result != RESULT_OK )
						return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to check the assumption level for 'possibly'" );
			...
			case WORD_PARAMETER_ADVERB_ASSUMPTION_PROBABLY:
				if( isSelfGeneratedAssumption )
					{
					if( ( specificationResult = checkAssumptionLevel( writeSpecificationItem ) ).result != RESULT_OK )
						return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to check the assumption level for 'probably'" );
				...
				if( !isArchivedAssignment )
					...
					if( pronounContextNr > NO_CONTEXT_NR )
						...
						if( ( pronounWordItem = myWordItem_->predefinedWordItem( definitionGrammarParameter ) ) == NULL )
							return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "I couldn't find the pronoun word with the definition grammar parameter" );
					...
					!writeSpecificationItem->hasGeneralizationCollection() )
						...
						if( myWordItem_->firstNonQuestionAssignmentItem( false, true, true, isNegative, isPossessive, relationContextNr, specificationWordItem ) == NULL )
							...
							if( ( predefinedWordItem = myWordItem_->predefinedWordItem( definitionGrammarParameter == WORD_PARAMETER_SINGULAR_VERB_WAS ? WORD_PARAMETER_SINGULAR_VERB_IS : WORD_PARAMETER_PLURAL_VERB_ARE ) ) == NULL )
								return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "I couldn't find the predefined word with word parameter ", definitionGrammarParameter );

							if( ( writeWordString_ = predefinedWordItem->wordTypeString( true, definitionGrammarWordTypeNr ) ) == NULL )
								return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "I couldn't get the word type string from the predefined word with word parameter ", definitionGrammarParameter );
							}
						}
					}
					...
					if( isGeneralization )
						...
						if( generalizationWordTypeNr == definitionGrammarWordTypeNr )
							...
							if( writeGeneralizationWordToSentence( ( isSpecificationGeneralization && !isPartOf && !hasFoundSingleSpecificationWord_ ), definitionGrammarWordTypeNr ) != RESULT_OK )
								return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to write the generalization word to the sentence" );
							}
						}
					else
						{
						if( isSpecification )
							...
							writeSpecificationItem->specificationWordTypeNr() == definitionGrammarWordTypeNr )
								...
								if( writeSpecificationWordToSentence( isSpecificationGeneralization, isWordTypeNumeral, isWordTypePluralNoun, isWritingCurrentSpecificationWordOnly, definitionGrammarWordTypeNr, writeSpecificationItem ) != RESULT_OK )
									return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to write a specification word to the sentence" );
								}
							}
						else
							{
							...
							writeSpecificationItem->relationWordTypeNr() == definitionGrammarWordTypeNr )
								...
								if( writeRelationWordToSentence( definitionGrammarWordTypeNr, writeSpecificationItem ) != RESULT_OK )
									return myWordItem_->addWriteResultErrorInWord( functionNameString, moduleNameString_, "I failed to write a relation word to the sentence" );
								}
							}
						}
					}
			...
			default:
				return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "I don't know how to handle the grammar definition with grammar parameter: ", definitionGrammarParameter );
			}
		...
		if( writeWordString_ != NULL )
			...
			if( isUnknownPluralOfNoun_ )
				...
				if( ( currentLanguageWordItem = commonVariables_->currentLanguageWordItem ) == NULL )
					return myWordItem_->startWriteResultErrorInWord( functionNameString, moduleNameString_, "The current language word item is undefined" );
