...
class JustificationList : private List
	...
	bool hasPrimarySpecification( unsigned short justificationTypeNr, SpecificationItem *primarySpecificationItem )
		{
	...
	ResultType replaceJustificationOfCorrectedAssumptionByOppositeQuestion( JustificationItem *oldJustificationItem, JustificationItem *replacingJustificationItem )
		{
		...
		if( oldJustificationItem != NULL )
			...
			if( replacingJustificationItem != NULL )
				...
				if( ( secondarySpecificationWordItem = replacingJustificationItem->secondarySpecificationWordItem() ) != NULL )
					{
					if( ( foundSpecificationItem = myWordItem()->firstAssignmentOrSpecificationItem( false, false, false, false, false, NO_QUESTION_PARAMETER, secondarySpecificationWordItem ) ) != NULL )
						{
								...
								if( ( attachedJustificationItem = firstJustificationItem->attachedJustificationItem() ) != NULL )
									...
									if( firstJustificationItem->changeAttachedJustification( NULL ) == RESULT_OK )
										...
										if( replaceOrDeleteJustification( attachedJustificationItem ) == RESULT_OK )
											{
											if( ( justificationResult = copyJustification( false, attachedJustificationItem->primarySpecificationItem(), NULL, attachedJustificationItem ) ).result == RESULT_OK )
												{
	...
	JustificationResultType addJustification( bool isActiveItem, bool isForcingNewJustification, unsigned short justificationTypeNr, unsigned short orderNr, unsigned int originalSentenceNr, SpecificationItem *primarySpecificationItem, SpecificationItem *anotherPrimarySpecificationItem, SpecificationItem *secondarySpecificationItem, SpecificationItem *anotherSecondarySpecificationItem, JustificationItem *attachedJustificationItem )
		{
		JustificationResultType justificationResult;
		unsigned short createOrderNr = orderNr;
			...
			foundJustificationItem->attachedJustificationItem() != attachedJustificationItem ) ) ) )
				...
				if( commonVariables()->currentItemNr < MAX_ITEM_NR )
					...
					foundJustificationItem == NULL )
						...
						hasPrimarySpecification( JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION, updatedPrimarySpecificationItem ) ) ) )
							createOrderNr++;
						}
						...
						if( ( firstJustificationItem = involvedSpecificationItem->firstJustificationItem() ) != NULL )
								...
								if( oldJustificationItem != NULL )
									...
									if( oldJustificationItem->isOlderItem() )
										...
										!involvedSpecificationItem->hasRelationContext() )
											...
											{
											if( replaceJustification( false, false, false, oldJustificationItem, attachJustificationItem, involvedSpecificationItem, NULL ) == RESULT_OK )
							...
							if( oldJustificationItem == NULL )
									...
									if( attachJustificationItem->hasAttachedJustification() )
										...
										if( !lastAttachedJustificationItem->hasAttachedJustification() )
											...
											if( previousAttachedJustificationItem != NULL )
												...
												if( ( justificationResult = copyJustification( false, lastAttachedJustificationItem->primarySpecificationItem(), attachJustificationItem, lastAttachedJustificationItem ) ).result == RESULT_OK )
													{
													if( ( createdJustificationItem = justificationResult.createdJustificationItem ) != NULL )
														...
														if( previousAttachedJustificationItem->changeAttachedJustification( createdJustificationItem ) == RESULT_OK )
															...
															if( replaceJustification( false, false, false, lastAttachedJustificationItem, createdJustificationItem, involvedSpecificationItem, NULL ) == RESULT_OK )
																{
																if( myWordItem()->recalculateAssumptions( false ) != RESULT_OK )
								...
								if( !isSkipUpdateJustification )
									{
									if( myWordItem()->updateJustificationInSpecifications( false, false, false, oldJustificationItem, attachJustificationItem ) != RESULT_OK )
			...
			if( involvedSpecificationItem != NULL )
				...
				if( oldJustificationItem->isOlderItem() )
					...
					if( oldJustificationItem->replacingJustificationItem == NULL )
							...
							if( oldJustificationItem != replacingJustificationItem )
								...
								replacingJustificationItem->attachedJustificationItem() == attachedJustificationItem ) ) )
									...
									if( oldJustificationItem->isActiveItem() )
										{
										if( replaceOrDeleteJustification( oldJustificationItem ) == RESULT_OK )
											{
											if( myWordItem()->updateJustificationInSpecifications( hasConfirmedSpecification, isExclusiveSpecification, isExclusiveGeneralization, oldJustificationItem, replacingJustificationItem ) == RESULT_OK )
												{
												if( replacingJustificationItem != NULL )
													{
													...
													if( updateReplacedJustifications( oldJustificationItem, replacingJustificationItem ) == RESULT_OK )
										...
										if( hasCorrectedAssumptionByOppositeQuestion )
											{
											if( replaceJustificationOfCorrectedAssumptionByOppositeQuestion( oldJustificationItem, replacingJustificationItem ) != RESULT_OK )
									...
									!attachedJustificationItem->isQuestionJustification() )
										...
										if( involvedSpecificationItem->changeFirstJustification( attachedJustificationItem ) != RESULT_OK )
											return addError( functionNameString, NULL, myWordItem()->anyWordTypeString(), "I failed to change the first justification item of the given corrected specification item to the attached justification item of the given old justification item" );
										}
									else
										{
								...
								return startError( functionNameString, NULL, myWordItem()->anyWordTypeString(), "The given old justification item and the given replacing justification item are the same justification item" );
							}
						else
						...
						return startError( functionNameString, NULL, myWordItem()->anyWordTypeString(), "The given old justification item is already replaced, because it has a replacing justification item" );
					}
				else
					return startError( functionNameString, NULL, myWordItem()->anyWordTypeString(), "The given old justification item isn't old" );
				}
			else
				return startError( functionNameString, NULL, myWordItem()->anyWordTypeString(), "The given involved specification item is undefined" );
			}
...
*/
					...
					searchItem->hasCurrentCreationSentenceNr() )
						...
						if( isReplacePrimarySpecification )
							...
							( involvedSpecificationItem = myWordItem()->justificationSpecificationItem( replacingSpecificationItem->isAssignment(), replacingSpecificationItem->isInactiveAssignment(), replacingSpecificationItem->isArchivedAssignment(), searchItem ) ) != NULL )
								{
								if( replaceJustification( hasConfirmedSpecificationInMainWord, false, false, searchItem, NULL, involvedSpecificationItem, NULL ) != RESULT_OK )
	...
	JustificationItem *questionJustificationItem( SpecificationItem *secondarySpecificationItem )
		{
		JustificationItem *searchItem = firstActiveJustificationItem();
