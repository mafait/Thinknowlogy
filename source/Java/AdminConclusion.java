...
class AdminConclusion
	...
	private void clearMissingNegativeConclusionVariables()
		{
											...
											hasGeneralizationWordFeminineProperNameEnding ) ) )
												...
												anotherPrimarySpecificationWordItem.lastCompoundCollectionNr( specificationWordTypeNr ) == anotherPrimarySpecificationCollectionNr ) )
													...
													primarySpecificationItem.isUserSpecification() )
														...
														generalizationWordItem.firstSelfGeneratedCheckSpecificationItem( true, isArchivedAssignment, isArchivedAssignment, false, false, false, Constants.NO_QUESTION_PARAMETER, primarySpecificationItem.specificationCollectionNr(), Constants.NO_CONTEXT_NR, specificationWordItem ) == null )
															{
															if( adminItem_.askSpecificationSubstitutionQuestion( isArchivedAssignment, isExclusiveSpecification, generalizationWordTypeNr, specificationWordTypeNr, generalizationContextNr, specificationContextNr, primarySpecificationItem, anotherPrimarySpecificationItem, generalizationWordItem, specificationWordItem ) != Constants.RESULT_OK )
											...
											if( anotherPrimarySpecificationItem.isExclusiveSpecification() )
												...
												primarySpecificationItem.specificationCollectionNr() != anotherPrimarySpecificationCollectionNr )
													...
													primarySpecificationItem.isUserSpecification() )
														{
														if( adminItem_.askSpecificationSubstitutionQuestion( isArchivedAssignment, isExclusiveSpecification, generalizationWordTypeNr, specificationWordTypeNr, generalizationContextNr, specificationContextNr, primarySpecificationItem, anotherPrimarySpecificationItem, generalizationWordItem, specificationWordItem ) != Constants.RESULT_OK )
															return adminItem_.addError( 1, moduleNameString_, "I failed to ask a specification substitution question about generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" with specification word \"" + specificationWordItem.anyWordTypeString() + "\" that is collected with itself" );
														}
													}
												else
													{
													...
													anotherPrimarySpecificationWordItem.lastCompoundCollectionNr( specificationWordTypeNr ) == anotherPrimarySpecificationCollectionNr )
														{
														if( drawNegativeConclusion( isArchivedAssignment, isSpecificationCollectedWithItself, generalizationWordTypeNr, specificationWordTypeNr, generalizationContextNr, specificationContextNr, primarySpecificationItem, anotherPrimarySpecificationItem, relatedSpecificationItem, generalizationWordItem ) != Constants.RESULT_OK )
															return adminItem_.addError( 1, moduleNameString_, "I failed to draw a negative conclusion about generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
														}
													}
												}
													...
													if( ( specificationResult = adminItem_.addSelfGeneratedSpecification( false, isArchivedAssignment, isArchivedAssignment, false, primarySpecificationItem.isExclusiveSpecification(), false, true, false, false, false, false, false, Constants.NO_ASSUMPTION_LEVEL, ( isArchivedAssignment && isUserSpecificationCollectedWithItself && primarySpecificationItem.isSelfGeneratedAssumption() ? Constants.JUSTIFICATION_TYPE_NEGATIVE_CONCLUSION : Constants.JUSTIFICATION_TYPE_NEGATIVE_ASSUMPTION ), Constants.JUSTIFICATION_TYPE_NEGATIVE_CONCLUSION, Constants.NO_PREPOSITION_PARAMETER, Constants.NO_QUESTION_PARAMETER, generalizationWordTypeNr, specificationWordTypeNr, Constants.WORD_TYPE_UNDEFINED, anotherPrimarySpecificationCollectionNr, generalizationContextNr, specificationContextNr, Constants.NO_CONTEXT_NR, ( isPrimarySpecificationHidden ? ( hasAnotherSecondarySpecificationWithCompoundCollection || userSpecificationItem == null ? anotherPrimarySpecificationItem : userSpecificationItem ) : ( isPrimarySameAsUserSpecification ? anotherPrimarySpecificationItem : primarySpecificationItem ) ), ( isPrimarySpecificationHidden ? oppositePossessiveDefinitionSpecificationItem : ( isPrimarySameAsUserSpecification ? null : anotherPrimarySpecificationItem ) ), ( isPrimarySpecificationHidden && userSpecificationItem != null ? ( hasAnotherSecondarySpecificationWithCompoundCollection ? userSpecificationItem : anotherPrimarySpecificationItem ) : ( isSelectingUserSpecification ? userSpecificationItem : secondarySpecificationItem ) ), ( isPrimarySpecificationHidden && userSpecificationItem != null ? ( hasAnotherSecondarySpecificationWithCompoundCollection ? anotherSecondarySpecificationItem : secondarySpecificationItem ) : ( isSelectingUserSpecification ? null : anotherSecondarySpecificationItem ) ), generalizationWordItem, anotherPrimarySpecificationWordItem, null ) ).result == Constants.RESULT_OK )
														...
														if( ( createdSpecificationItem = specificationResult.createdSpecificationItem ) == null )
															...
															if( generalizationWordItem.hasCorrectedAssumptionByKnowledge() )
																...
																anotherPrimarySpecificationItem.hasSpecificationCompoundCollection() )
																	{
																	if( ( questionJustificationItem = generalizationWordItem.secondarySpecificationJustificationItem( false, Constants.JUSTIFICATION_TYPE_SPECIFICATION_SUBSTITUTION_QUESTION, anotherPrimarySpecificationItem ) ) == null )
																		{
																		if( isSelectingUserSpecification )
																			{
														...
														return adminItem_.addError( 1, moduleNameString_, "I failed to create a negative conclusion about generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" with specification word \"" + anotherPrimarySpecificationWordItem.anyWordTypeString() + "\"" );
													}
												}
											else
												return adminItem_.startError( 1, moduleNameString_, "The given related specification item has no specification word item" );
											}
										}
									else
										return adminItem_.startError( 1, moduleNameString_, "The given another primary specification item has no specification word item" );
										...
										if( ( specificationResult = adminItem_.addSelfGeneratedSpecification( hasFeminineProperNameEnding, isAssignment, isArchivedAssignment, false, isExclusiveSpecification, isForcingNewJustification, false, false, !isPossessive, isUniqueUserRelation, false, false, Constants.NO_ASSUMPTION_LEVEL, Constants.JUSTIFICATION_TYPE_POSSESSIVE_REVERSIBLE_ASSUMPTION, Constants.JUSTIFICATION_TYPE_POSSESSIVE_REVERSIBLE_CONCLUSION, Constants.WORD_PARAMETER_PREPOSITION_OF, Constants.NO_QUESTION_PARAMETER, relationWordTypeNr, Constants.WORD_TYPE_NOUN_SINGULAR, generalizationWordTypeNr, specificationCollectionNr, Constants.NO_CONTEXT_NR, specificationContextNr, generalizationContextNr, null, null, secondarySpecificationItem, null, relationWordItem, specificationWordItem, generalizationWordItem ) ).result == Constants.RESULT_OK )
											...
											if( conclusionSpecificationItem == null )
												...
												if( specificationResult.createdSpecificationItem == null )
													{
													if( generalizationWordItem.replaceOrDeleteObsoleteJustification() == Constants.RESULT_OK )
													...
													if( isPossessive )
														...
														if( adminItem_.makePossessiveConditionalSpecificationAssumption( isArchivedAssignment, isExclusiveSpecification, true, generalizationWordTypeNr, specificationWordTypeNr, relationWordTypeNr, createdOrFoundSpecificationItem, null, generalizationWordItem, relationWordItem ) == Constants.RESULT_OK )
															...
															( collectedWithItselfCommonWordItem = createdOrFoundSpecificationItem.collectedWithItselfCommonWordItem() ) == null )
																...
																if( generalizationWordItem.bestMatchingRelationContextNrSpecificationItem( false, true, Constants.NO_QUESTION_PARAMETER, specificationWordItem, relationWordItem ) == null )
																	{
																...
																if( ( primarySpecificationItem = relationWordItem.bestMatchingRelationContextNrSpecificationItem( false, false, Constants.NO_QUESTION_PARAMETER, collectedWithItselfCommonWordItem, generalizationWordItem ) ) != null &&
																			...
																			if( relationWordItem.replaceOrDeleteSpecification( conclusionSpecificationItem, foundSpecificationItem ) == Constants.RESULT_OK )
																				...
																				while( ( obsoleteJustificationItem = relationWordItem.obsoleteJustificationItem() ) != null )
																					...
																					if( relationWordItem.replaceOrDeleteJustification( obsoleteJustificationItem ) != Constants.RESULT_OK )
																						return adminItem_.addError( 1, moduleNameString_, "I failed to delete an obsolete assumption justification item in relation word \"" + relationWordItem.anyWordTypeString() + "\"" );
																					}
																				}
																			else
	...
	protected byte drawSpecificationSubstitutionConclusionOrAskQuestion( boolean isAssumption, boolean isInactiveAssignment, boolean isArchivedAssignment, boolean isExclusiveSpecification, short questionParameter, short generalizationWordTypeNr, short specificationWordTypeNr, short relationWordTypeNr, int generalizationContextNr, int specificationContextNr, WordItem generalizationWordItem, WordItem specificationWordItem, WordItem relationWordItem )
		...
		boolean isUserSpecificationWordCollectedWithItself = false;
													...
													if( ( specificationResult = adminItem_.addSelfGeneratedSpecification( true, isArchivedAssignment, isArchivedAssignment, false, false, false, false, false, false, false, false, false, Constants.NO_ASSUMPTION_LEVEL, Constants.JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION, Constants.JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION, foundPrimarySpecificationItem.prepositionParameter(), Constants.NO_QUESTION_PARAMETER, generalizationWordTypeNr, specificationWordTypeNr, ( isIncludingRelationWord ? relationWordTypeNr : Constants.WORD_TYPE_UNDEFINED ), Constants.NO_COLLECTION_NR, generalizationContextNr, specificationContextNr, Constants.NO_CONTEXT_NR, ( isFoundPrimaryHiddenSpecification ? userSpecificationItem : foundPrimarySpecificationItem ), ( isFoundPrimaryHiddenSpecification ? oppositePossessiveDefinitionSpecificationItem : null ), currentSpecificationItem, null, generalizationWordItem, currentSpecificationWordItem, ( isIncludingRelationWord ? relationWordItem : null ) ) ).result == Constants.RESULT_OK )
														...
														if( ( createdSpecificationItem = specificationResult.createdSpecificationItem ) == null )
															...
															if( generalizationWordItem.replaceOrDeleteObsoleteJustification() == Constants.RESULT_OK )
																...
																return adminItem_.addError( 1, moduleNameString_, "I failed to replace or delete an obsolete assumption justification item in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
															}
														else
															{
															...
															missingNegativeConclusionSpecificationCollectionNr_ == currentSpecificationCollectionNr )
																...
																if( drawSpecificationSubstitutionConclusionOrAskQuestion( isAssumption, isInactiveAssignment, isArchivedAssignment, isExclusiveSpecification, questionParameter, generalizationWordTypeNr, specificationWordTypeNr, relationWordTypeNr, generalizationContextNr, specificationContextNr, generalizationWordItem, specificationWordItem, relationWordItem ) != Constants.RESULT_OK )
														...
														relationContextNr > Constants.NO_CONTEXT_NR )
															{
															if( drawPossessiveReversibleConclusion( isArchivedAssignment, false, false, false, Constants.NO_ASSUMPTION_LEVEL, generalizationWordTypeNr, specificationWordTypeNr, relationWordTypeNr, specificationContextNr, relationContextNr, generalizationWordItem, currentSpecificationWordItem, relationWordItem ) != Constants.RESULT_OK )
						...
						do	{
							if( ( currentGeneralizationWordItem = currentGeneralizationItem.generalizationWordItem() ) != null )
								...
								if( ( foundSpecificationItem = currentGeneralizationWordItem.bestMatchingSpecificationWordSpecificationItem( true, true, true, false, false, Constants.NO_COLLECTION_NR, generalizationContextNr, specificationContextNr, relationContextNr, specificationWordItem ) ) != null )
									{
									if( drawSpecificationSubstitutionConclusionOrAskQuestion( foundSpecificationItem.hasAssumptionLevel(), foundSpecificationItem.isInactiveAssignment(), foundSpecificationItem.isArchivedAssignment(), foundSpecificationItem.isExclusiveSpecification(), Constants.NO_QUESTION_PARAMETER, foundSpecificationItem.generalizationWordTypeNr(), specificationWordTypeNr, Constants.WORD_TYPE_UNDEFINED, generalizationContextNr, specificationContextNr, currentGeneralizationWordItem, specificationWordItem, null ) == Constants.RESULT_OK )
										{
										if( !CommonVariables.hasShownWarning )
											{
