...
class AdminReasoning
		...
		if( generalizationWordItem != null )
			...
			if( primarySpecificationWordItem != null )
				...
				if( questionSpecificationWordItem != null )
					...
					return adminItem_.startErrorInItem( 1, moduleNameString_, "The given question specification word item is undefined" );
				}
			else
				return adminItem_.startErrorInItem( 1, moduleNameString_, "The given primary specification word item is undefined" );
			}
		else
	...
	private byte processJustification( boolean hasFoundRelationContext, boolean hasFoundSelfGeneratedAssumption, boolean hasRelationWord, short justificationTypeNr, JustificationItem createdOrFoundJustificationItem, SpecificationItem foundSpecificationItem, SpecificationItem primarySpecificationItem, SpecificationItem secondarySpecificationItem, WordItem generalizationWordItem, WordItem relationWordItem )
		{
		...
		if( createdOrFoundJustificationItem != null )
			...
			if( foundSpecificationItem != null )
				...
				if( generalizationWordItem != null )
						...
						switch( justificationTypeNr )
								...
								if( relationWordItem != null )
									...
									if( secondarySpecificationItem.isSelfGeneratedAssumption() )
										...
										( assumptionSpecificationItem = relationWordItem.firstSelfGeneratedCheckSpecificationItem( false, secondarySpecificationItem.isArchivedAssignment(), secondarySpecificationItem.isNegative(), isPossessiveSecondarySpecification, true, secondarySpecificationWordItem, generalizationWordItem ) ) != null )
											{
											if( ( existingJustificationItem = generalizationWordItem.olderSecondarySpecificationJustificationItem( Constants.JUSTIFICATION_TYPE_OPPOSITE_POSSESSIVE_CONDITIONAL_SPECIFICATION_ASSUMPTION, assumptionSpecificationItem ) ) != null )
												{
												if( generalizationWordItem.replaceJustification( existingJustificationItem, createdOrFoundJustificationItem, foundSpecificationItem ) == Constants.RESULT_OK )
													...
													return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace an existing justification item in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
												}
											}
										}
									}
							...
							case Constants.JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION:
								...
								secondarySpecificationItem.isUserSpecification() )
									{
									if( ( existingJustificationItem = generalizationWordItem.olderPrimarySpecificationJustificationItem( false, true, Constants.JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION, secondarySpecificationItem.specificationCollectionNr(), primarySpecificationItem ) ) == null )
										{
										if( ( obsoleteJustificationItem = generalizationWordItem.secondaryAssumptionJustificationItem( !isPossessiveSecondarySpecification, secondarySpecificationWordItem ) ) != null )
											{
											if( ( justificationResult = generalizationWordItem.addJustification( false, false, Constants.JUSTIFICATION_TYPE_EXCLUSIVE_SPECIFICATION_SUBSTITUTION_ASSUMPTION, obsoleteJustificationItem.orderNr, obsoleteJustificationItem.originalSentenceNr(), null, null, secondarySpecificationItem, null, null ) ).result == Constants.RESULT_OK )
												{
												if( justificationResult.createdJustificationItem != null )
													...
													if( generalizationWordItem.replaceJustification( obsoleteJustificationItem, justificationResult.createdJustificationItem, foundSpecificationItem ) != Constants.RESULT_OK )
														return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace an obsolete justification item without primary specification in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
													}
												else
													return adminItem_.startErrorInItem( 1, moduleNameString_, "I couldn't find or create a justification without primary specification" );
												}
											else
												return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to add a justification without primary specification to generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
											}
										else
											{
											// Skip if justification already exists as an opposite possessive conditional specification assumption
											if( generalizationWordItem.olderSecondarySpecificationJustificationItem( Constants.JUSTIFICATION_TYPE_OPPOSITE_POSSESSIVE_CONDITIONAL_SPECIFICATION_ASSUMPTION, secondarySpecificationItem ) != null )
												isDeletingCreatedJustification = true;
											}
										}
									else
										...
										if( generalizationWordItem.replaceJustification( existingJustificationItem, createdOrFoundJustificationItem, foundSpecificationItem ) == Constants.RESULT_OK )
											...
											return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace an existing justification by the created justification item in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
										}
									}
								...
								primarySpecificationItem != null )
									...
									if( ( existingJustificationItem = foundSpecificationItem.differentAssumptionLevelSpecificationSubstitutionJustificationItem( primarySpecificationItem, secondarySpecificationItem ) ) == null )
										...
										if( primarySpecificationItem.assumptionLevel() < existingJustificationItem.primarySpecificationAssumptionLevel() )
											...
											if( generalizationWordItem.replaceJustification( existingJustificationItem, createdOrFoundJustificationItem, foundSpecificationItem ) != Constants.RESULT_OK )
												return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace an older generalization assumption by the created justification item in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
											}
										else
											isDeletingCreatedJustification = true;
										}
								...
								( specificationSubstitutionPartOfAssumptionJustificationItem = foundSpecificationItem.firstJustificationItem( Constants.JUSTIFICATION_TYPE_SPECIFICATION_SUBSTITUTION_PART_OF_ASSUMPTION ) ) != null )
									...
									if( specificationSubstitutionPartOfAssumptionJustificationItem.hasHiddenPrimarySpecification() )
										...
										if( specificationSubstitutionPartOfAssumptionJustificationItem.changePrimarySpecification( primarySpecificationItem ) == Constants.RESULT_OK )
											...
											return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace a hidden primary specification of the attached justification item of the found specification substitution 'part of' assumption justification item" );
										}
									}
							}
						}
					...
					if( isDeletingCreatedJustification )
						...
						if( generalizationWordItem.replaceOrDeleteJustification( createdOrFoundJustificationItem ) != Constants.RESULT_OK )
							return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete the created justification item in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
						}
					else
						{
						if( isAttachingJustification )
							...
							if( generalizationWordItem.attachJustification( createdOrFoundJustificationItem, foundSpecificationItem ) != Constants.RESULT_OK )
								adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to attach a justification to a self-generated specification to generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
							}
						}
					}
				else
				...
				return adminItem_.startErrorInItem( 1, moduleNameString_, "The given found specification item is undefined" );
			}
		else
			return adminItem_.startErrorInItem( 1, moduleNameString_, "The given created or found justification item is undefined" );
									...
									if( generalizationWordItem.isUserGeneralizationWord )
										...
										nonCompoundSpecificationCollectionNr > Constants.NO_COLLECTION_NR )
											...
											if( generalizationWordItem.checkForSpecificationConflict( isArchivedAssignment, isExclusiveSpecification, false, false, specificationWordTypeNr, nonCompoundSpecificationCollectionNr, generalizationContextNr, specificationContextNr, secondaryRelationContextNr, secondarySpecificationWordItem ) == Constants.RESULT_OK )
												...
												if( ( specificationResult = generalizationWordItem.findQuestionToBeAdjustedByCompoundCollection( false, false, Constants.WORD_PARAMETER_SINGULAR_VERB_IS, secondarySpecificationCollectionNr, secondaryRelationContextNr, specificationWordItem ) ).result == Constants.RESULT_OK )
													{
													if( ( adjustedQuestionSpecificationItem = specificationResult.adjustedQuestionSpecificationItem ) != null )
														...
														if( generalizationWordItem.writeUpdatedSpecification( true, false, false, adjustedQuestionSpecificationItem ) == Constants.RESULT_OK )
															...
															return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write an adjusted compound question in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
														}
													}
												else
													return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to find a question to be adjusted by a compound collection in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" by compound collection" );
												}
											else
											...
											if( checkForConflictWithUserSpecification( generalizationWordItem, specificationWordItem, secondarySpecificationWordItem ) == Constants.RESULT_OK )
												...
												return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to check for a conflict with the user specification in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
											}
										}
												...
												if( ( createdSpecificationItem = specificationResult.createdSpecificationItem ) == null )
													...
													if( hasAdjustedQuestion_ )
														...
														if( generalizationWordItem.replaceObsoleteJustificationsOfAdjustedQuestion( specificationResult.createdJustificationItem, specificationResult.foundSpecificationItem ) != Constants.RESULT_OK )
															return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace obsolete justifications from an adjusted compound question" );
														}
													}
												else
													{
													...
													secondarySpecificationWordItem != specificationWordItem )
														...
														if( generalizationWordItem.replaceOrDeleteSpecification( foundSpecificationItem, createdSpecificationItem ) != Constants.RESULT_OK )
															return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to replace an invalid assumption specification in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
														}
													}
												}
											else
												return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to ask a specification substitution question about generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" with specification word \"" + secondarySpecificationWordItem.anyWordTypeString() + "\"" );
											}
										}
									}
								}
			...
			if( specificationWordItem != null )
				...
				!secondarySpecificationItem.isSelfGeneratedAssumption() ) ) )
					...
					if( adminItem_.isConclusion( conclusionJustificationTypeNr ) )
						...
						adminItem_.startErrorInItem( 1, moduleNameString_, "The given conclusion justification type isn't a conclusion" );
					}
					...
					if( ( justificationResult = generalizationWordItem.addJustification( hasFeminineOrMasculineProperNameEnding, isForcingNewJustification, justificationTypeNr, Constants.NO_ORDER_NR, CommonVariables.currentSentenceNr, primarySpecificationItem, anotherPrimarySpecificationItem, secondarySpecificationItem, anotherSecondarySpecificationItem, null ) ).result == Constants.RESULT_OK )
						...
						if( ( createdOrFoundJustificationItem = ( createdJustificationItem == null ? foundJustificationItem : createdJustificationItem ) ) != null )
							...
							if( ( specificationResult = adminItem_.addSpecification( isAssignment, false, isArchivedAssignment, isEveryGeneralization, isExclusiveSpecification, isNegative, isPartOf, isPossessive, isSpecificationGeneralization, ( justificationTypeNr == Constants.JUSTIFICATION_TYPE_UNIQUE_RELATION_ASSUMPTION || justificationTypeNr == Constants.JUSTIFICATION_TYPE_UNIQUE_RELATION_CONCLUSION ), prepositionParameter, questionParameter, assumptionLevel, generalizationWordTypeNr, specificationWordTypeNr, relationWordTypeNr, specificationCollectionNr, generalizationContextNr, specificationContextNr, relationContextNr, createdOrFoundJustificationItem, generalizationWordItem, specificationWordItem, relationWordItem, null ) ).result == Constants.RESULT_OK )
									...
									if( createdSpecificationItem == null )
										...
										if( foundSpecificationItem != null )
											...
											if( foundSpecificationItem.isSelfGenerated() )
												...
												adminItem_.nContextWordsInAllWords( foundSpecificationItem.relationContextNr(), specificationWordItem ) == 1 ) ) ) ) )
													{
													if( processJustification( hasFoundRelationContext, hasFoundSelfGeneratedAssumption, hasRelationWord, justificationTypeNr, createdOrFoundJustificationItem, foundSpecificationItem, primarySpecificationItem, secondarySpecificationItem, generalizationWordItem, relationWordItem ) == Constants.RESULT_OK )
														{
														if( hasRelationWord &&
														isAssumption &&
														!isPossessive &&
														...
														!specificationWordItem.isNounWordCollectedWithItself() )
															...
															if( generalizationWordItem.recalculateAssumptionsInWord() != Constants.RESULT_OK )
																adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to recalculate the assumptions of generalization word \"" + generalizationWordItem.anyWordTypeString() + "\"" );
															}
														}
													else
														adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to process the created justification" );
													}
												}
											else
												adminItem_.startErrorInItem( 1, moduleNameString_, "I found a user specification, whilst trying to create a self-generated specification" );
											}
										else
											adminItem_.startErrorInItem( 1, moduleNameString_, "I couldn't find or create a specification" );
										}
									else
										{
										...
										generalizationWordItem.hasConfirmedSpecification() )
											...
											if( !createdSpecificationItem.isHiddenSpecification() )
												...
												if( generalizationWordItem.writeSelectedSpecification( true, true, false, false, Constants.NO_ANSWER_PARAMETER, createdSpecificationItem ) == Constants.RESULT_OK )
													...
													CommonVariables.writeSentenceStringBuffer.length() == 0 )
														adminItem_.startErrorInItem( 1, moduleNameString_, "Integrity error! I couldn't write the self-generated specification with generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" and specification word \"" + specificationWordItem.anyWordTypeString() + "\". I guess, the implementation of my writing modules is insufficient to write this particular sentence structure" );
													}
												else
													adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write the self-generated specification in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" to check the writing integrity" );
												}
											}
										else
											{
											...
											foundSpecificationItem.relationContextNr() == createdRelationContextNr )
												adminItem_.startErrorInItem( 1, moduleNameString_, "Integrity error! A specification was created in generalization word \"" + generalizationWordItem.anyWordTypeString() + "\" while this specification was found as well" );
											}
										}
								...
								adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to create a specification" );
							}
						else
							adminItem_.startErrorInItem( 1, moduleNameString_, "I couldn't find or create a justification" );
						}
					else
						adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to add a justification" );
					}
				}
			else
