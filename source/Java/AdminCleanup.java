...
class AdminCleanup
	...
	private byte decrementItemNrRange( int decrementSentenceNr, int startDecrementItemNr, int decrementOffset )
		...
		if( ( wordList = adminItem_.wordList ) != null )
			...
			if( wordList.decrementItemNrRangeInWordList( decrementSentenceNr, startDecrementItemNr, decrementOffset ) != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to decrement item number range in my word list" );
			}
		...
		for( short adminListNr : Constants.AdminLists )
			...
			if( adminItem_.adminListArray[adminListNr] != null )
				...
				if( adminItem_.adminListArray[adminListNr].decrementItemNrRangeInList( decrementSentenceNr, startDecrementItemNr, decrementOffset ) != Constants.RESULT_OK )
					return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to decrement item number range" );
				}
			}
	...
	private byte decrementSentenceNrs( int startSentenceNr )
		...
		if( ( wordList = adminItem_.wordList ) != null )
			...
			if( wordList.decrementSentenceNrsInWordList( startSentenceNr ) != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to decrement the sentence numbers from the current sentence number in my word list" );
			}
		...
		for( short adminListNr : Constants.AdminLists )
			...
			if( adminItem_.adminListArray[adminListNr] != null )
				...
				if( adminItem_.adminListArray[adminListNr].decrementSentenceNrsInList( startSentenceNr ) != Constants.RESULT_OK )
					return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to decrement the sentence numbers from the current sentence number in one of my lists" );
				}
			}
							...
							if( unusedWordTypeItem.hasCurrentCreationSentenceNr() )
								...
								( singularNounWordTypeItem = unusedReadWordItem.activeWordTypeItem( false, Constants.WORD_TYPE_NOUN_SINGULAR ) ) != null )
									...
									if( singularNounWordTypeItem.createNewWordTypeString( pluralNounString ) != Constants.RESULT_OK )
										return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to create a new word string for a singular noun word type of an active read word" );
									}
									...
									if( ( nReadWordReferences = readResult.nReadWordReferences ) >= 1 )
										...
										if( nReadWordReferences == 1 )
											...
											if( unusedReadWordItem.deleteWordType( unusedReadItem.wordTypeNr() ) != Constants.RESULT_OK )
												return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete an unused word type item" );
											}
										}
									else
										return adminItem_.startErrorInItem( 1, moduleNameString_, "I've found an invalid number of read word references" );
									}
								else
									return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to get the number of read word references" );
								}
							}
						else
							return adminItem_.startErrorInItem( 1, moduleNameString_, "I couldn't find the word type of an active read word" );
						}
					}
	...
	private byte deleteUnusedWordTypes( boolean isDeleteAllActiveWordTypes )
		...
		if( deleteUnusedWordTypes( false, isDeleteAllActiveWordTypes ) == Constants.RESULT_OK )
			{
			// From inactive read items
			if( deleteUnusedWordTypes( true, isDeleteAllActiveWordTypes ) != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete the active unused word types" );
			}
		else
			return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete the inactive unused word types" );
			...
			for( short adminListNr : Constants.AdminLists )
				...
				if( adminItem_.adminListArray[adminListNr] != null )
					...
					if( adminItem_.adminListArray[adminListNr].redoCurrentSentenceInList() != Constants.RESULT_OK )
						return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to redo the current sentence" );
					}
				}
			}
		else
			return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to redo the current sentence in my word list" );
	...
	private byte removeFirstRangeOfDeletedItems()
		...
		if( ( wordList = adminItem_.wordList ) != null )
			...
			if( wordList.removeFirstRangeOfDeletedItemsInWordList() != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to remove the first deleted items in my word list" );
			}
		...
		CommonVariables.nDeletedItems == 0 )
			...
			if( adminItem_.adminListArray[adminListNr] != null )
				...
				if( adminItem_.adminListArray[adminListNr].removeFirstRangeOfDeletedItemsInList() != Constants.RESULT_OK )
					return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to remove the first deleted items" );
				}
	...
	private byte rollbackDeletedRedoInfo()
		...
		if( ( wordList = adminItem_.wordList ) != null )
			...
			if( wordList.rollbackDeletedRedoInfoInWordList() != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete the current redo info of my words in my word list" );
			}
			...
			for( short adminListNr : Constants.AdminLists )
				...
				if( adminItem_.adminListArray[adminListNr] != null )
					...
					if( adminItem_.adminListArray[adminListNr].undoCurrentSentenceInList() != Constants.RESULT_OK )
						return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to undo the current sentence" );
					}
				}
			}
		else
			return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to undo the current sentence in my word list" );
			...
			if( removeFirstRangeOfDeletedItems() == Constants.RESULT_OK )
				...
				previousRemoveSentenceNr != CommonVariables.removeSentenceNr )
					...
					if( CommonVariables.highestInUseSentenceNr < previousRemoveSentenceNr )
						...
						if( decrementSentenceNrs( previousRemoveSentenceNr ) == Constants.RESULT_OK )
							...
							return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to decrement the sentence numbers from the current sentence number" );
						}
					}
				...
				if( CommonVariables.nDeletedItems > 0 )
					...
					if( decrementItemNrRange( CommonVariables.removeSentenceNr, CommonVariables.removeStartItemNr, CommonVariables.nDeletedItems ) == Constants.RESULT_OK )
						...
						return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to decrement item number range" );
					}
				}
			else
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to remove the first deleted items" );
			}
	...
	protected byte deleteUnusedInterpretations( boolean isDeleteAllActiveWordTypes )
		...
		if( ( readList = adminItem_.readList ) != null )
			...
			if( ( wordList = adminItem_.wordList ) != null )
				{
				// From active read items
				if( deleteUnusedWordTypes( isDeleteAllActiveWordTypes ) == Constants.RESULT_OK )
					{
					while( unusedReadItem != null )
						{
								...
								unusedWordItem.hasCurrentCreationSentenceNr() )
									...
									adminListNr < Constants.NUMBER_OF_ADMIN_LISTS )
										...
										if( adminItem_.adminListArray[adminListNr] != null )
											...
											if( ( referenceResult = adminItem_.adminListArray[adminListNr].findWordReference( unusedWordItem ) ).result != Constants.RESULT_OK )
												return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to find a reference to an active word \"" + unusedWordItem.anyWordTypeString() + "\" in one of my lists" );
											}
									...
									if( !referenceResult.hasFoundWordReference )
										...
										if( wordList.deleteItem( false, unusedWordItem ) != Constants.RESULT_OK )
											return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete a word item" );
										}
									}
					...
					while( ( unusedReadItem = adminItem_.firstInactiveReadItem() ) != null )
						...
						if( readList.deleteItem( false, unusedReadItem ) == Constants.RESULT_OK )
							...
							unusedWordItem.hasCurrentCreationSentenceNr() )
								...
								adminListNr < Constants.NUMBER_OF_ADMIN_LISTS )
									...
									if( adminItem_.adminListArray[adminListNr] != null )
										...
										if( ( referenceResult = adminItem_.adminListArray[adminListNr].findWordReference( unusedWordItem ) ).result != Constants.RESULT_OK )
											return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to find a reference to an inactive word \"" + unusedWordItem.anyWordTypeString() + "\" in one of my lists" );
										}
							...
							return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete an unused (inactive) read word" );
						}
					}
				else
					return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete the unused word types" );
				}
			else
	...
	protected byte deleteSentences( boolean isAvailableForRollback, int lowestSentenceNr )
		...
		if( ( wordList = adminItem_.wordList ) != null )
			...
			if( wordList.deleteSentencesInWordList( isAvailableForRollback, lowestSentenceNr ) != Constants.RESULT_OK )
				return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete sentences in my word list" );
			}
		...
		for( short adminListNr : Constants.AdminLists )
			...
			if( adminItem_.adminListArray[adminListNr] != null )
				...
				if( adminItem_.adminListArray[adminListNr].deleteSentencesInList( isAvailableForRollback, lowestSentenceNr ) != Constants.RESULT_OK )
					return adminItem_.addErrorInItem( adminItem_.adminListChar( adminListNr ), 1, moduleNameString_, "I failed to delete sentences in a list" );
				}
			}
		...
		if( deleteAllTemporaryLists() == Constants.RESULT_OK )
			...
			if( CommonVariables.currentSentenceNr <= adminItem_.firstSentenceNrOfCurrentUser() )
				...
				if( Presentation.writeInterfaceText( false, Constants.PRESENTATION_PROMPT_NOTIFICATION, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_NO_SENTENCES_TO_UNDO ) == Constants.RESULT_OK )
					...
					return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write an interface notification about another user" );
				}
			else
				{
				if( previousSentenceNr == CommonVariables.currentSentenceNr )
				...
				if( undoCurrentSentence() == Constants.RESULT_OK )
					...
					if( Presentation.writeInterfaceText( Constants.PRESENTATION_PROMPT_NOTIFICATION, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_I_HAVE_UNDONE_SENTENCE_NR_START, CommonVariables.currentSentenceNr, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_I_HAVE_UNDONE_SENTENCE_NR_END ) == Constants.RESULT_OK )
						...
						return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write an interface notification about having undone a sentence" );
					}
				else
					return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to undo the current sentence" );
				}
			}
		else
			return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete all temporary lists" );
			...
			if( CommonVariables.currentSentenceNr > highestSentenceNr() )
				...
				if( redoCurrentSentence() == Constants.RESULT_OK )
					...
					if( Presentation.writeInterfaceText( Constants.PRESENTATION_PROMPT_NOTIFICATION, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_I_HAVE_REDONE_SENTENCE_NR_START, CommonVariables.currentSentenceNr, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_I_HAVE_REDONE_SENTENCE_NR_END ) == Constants.RESULT_OK )
						...
						return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write an interface notification about having redone a sentence" );
					}
				else
					return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to redo the current sentence" );
				}
			else
				{
				if( Presentation.writeInterfaceText( false, Constants.PRESENTATION_PROMPT_NOTIFICATION, Constants.INTERFACE_IMPERATIVE_NOTIFICATION_NO_SENTENCES_TO_REDO ) != Constants.RESULT_OK )
					return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to write an interface notification about no sentences to redo" );
				}
			}
		else
			return adminItem_.addErrorInItem( 1, moduleNameString_, "I failed to delete all temporary lists" );
